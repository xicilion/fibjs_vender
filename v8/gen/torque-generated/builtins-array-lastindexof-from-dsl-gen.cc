...
compiler::TNode<Object> ArrayLastindexofBuiltinsFromDSLAssembler::TryFastArrayLastIndexOf(compiler::TNode<Context> p_context, compiler::TNode<JSReceiver> p_receiver, compiler::TNode<Object> p_searchElement, compiler::TNode<Number> p_from, compiler::CodeAssemblerLabel* label_Slow) {
  ...
  if (block10.is_used()) {
    ...
    USE(tmp68);
    tmp68 = ca_.UncheckedCast<Smi>(ArrayLastindexofBuiltinsFromDSLAssembler(state_).FastArrayLastIndexOf16FixedDoubleArray(compiler::TNode<Context>{tmp60}, compiler::TNode<JSArray>{tmp64}, compiler::TNode<Smi>{tmp66}, compiler::TNode<Object>{tmp62}));
    ca_.Goto(&block2, tmp60, tmp61, tmp62, tmp63, tmp68);
  }
  ...
  if (block2.is_used()) {
    ...
    ca_.Bind(&block2, &tmp69, &tmp70, &tmp71, &tmp72, &tmp73);
    ca_.SetSourcePosition("../../src/builtins/array-lastindexof.tq", 83);
    ca_.Goto(&block11, tmp69, tmp70, tmp71, tmp72, tmp73);
  }
  ...
  if (block1.is_used()) {
    ...
    ca_.Bind(&block11, &tmp74, &tmp75, &tmp76, &tmp77, &tmp78);
  return compiler::TNode<Object>{tmp78};
}
...
compiler::TNode<Smi> ArrayLastindexofBuiltinsFromDSLAssembler::FastArrayLastIndexOf10FixedArray(compiler::TNode<Context> p_context, compiler::TNode<JSArray> p_array, compiler::TNode<Smi> p_from, compiler::TNode<Object> p_searchElement) {
  ...
  if (block11.is_used()) {
    ...
    ca_.Goto(&block1, tmp57, tmp58, tmp59, tmp60, tmp62);
  }
  ...
  if (block5.is_used()) {
    ...
    ca_.Goto(&block1, tmp87, tmp88, tmp89, tmp90, tmp93);
  }
  ...
  if (block1.is_used()) {
    ...
    ca_.Bind(&block1, &tmp94, &tmp95, &tmp96, &tmp97, &tmp98);
    ca_.SetSourcePosition("../../src/builtins/array-lastindexof.tq", 28);
    ca_.Goto(&block13, tmp94, tmp95, tmp96, tmp97, tmp98);
  }
    ...
    ca_.Bind(&block13, &tmp99, &tmp100, &tmp101, &tmp102, &tmp103);
  return compiler::TNode<Smi>{tmp103};
}
