...
void TypedArraySliceBuiltinsFromDSLAssembler::FastCopy(compiler::TNode<JSTypedArray> p_src, compiler::TNode<JSTypedArray> p_dest, compiler::TNode<IntPtrT> p_k, compiler::TNode<Smi> p_count, compiler::CodeAssemblerLabel* label_IfSlow) {
  ...
  if (block14.is_used()) {
    ...
    USE(tmp193);
    tmp193 = ca_.UncheckedCast<RawPtrT>(TypedArrayBuiltinsAssembler(state_).LoadDataPtr(compiler::TNode<JSTypedArray>{tmp181}));
    TypedArrayBuiltinsAssembler(state_).CallCMemmove(compiler::TNode<RawPtrT>{tmp193}, compiler::TNode<RawPtrT>{tmp192}, compiler::TNode<UintPtrT>{tmp188});
    ca_.SetSourcePosition("../../src/builtins/typed-array-slice.tq", 15);
    ca_.SetSourcePosition("../../src/builtins/typed-array-slice.tq", 13);
    ca_.Goto(&block2, tmp180, tmp181, tmp182, tmp183);
  }
  ...
  if (block2.is_used()) {
    ...
    ca_.Bind(&block2, &tmp194, &tmp195, &tmp196, &tmp197);
    ca_.Goto(&block19, tmp194, tmp195, tmp196, tmp197);
  }
  ...
  if (block1.is_used()) {
    ...
    ca_.Bind(&block19, &tmp198, &tmp199, &tmp200, &tmp201);
}
