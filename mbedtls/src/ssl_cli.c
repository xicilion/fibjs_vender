...
static int ssl_parse_server_hello( mbedtls_ssl_context *ssl )
{
  ...
  buf = ssl->in_msg;
  printf("%s\n", "-----ssl_parse_server_hello begin");
  if ( ( ret = mbedtls_ssl_read_record( ssl ) ) != 0 )
  {
    MBEDTLS_SSL_DEBUG_RET( 1, "mbedtls_ssl_read_record", ret );
    return ( ret );
  }
  printf("%s\n", "-----ssl_parse_server_hello end");
...
int mbedtls_ssl_handshake_client_step( mbedtls_ssl_context *ssl )
{
...
#endif
  printf("----mbedtls_ssl_handshake_client_step client_step res:%d and state:%d\n", ret, ssl->state);
  switch ( ssl->state )
  {
  ...
  case MBEDTLS_SSL_SERVER_HELLO:
    printf("----mbedtls_ssl_handshake_client_step state MBEDTLS_SSL_SERVER_HELLO\n");
  ...
  default:
    printf("----mbedtls_ssl_handshake_client_step client_step default:%d\n", ret);
    MBEDTLS_SSL_DEBUG_MSG( 1, ( "invalid state %d", ssl->state ) );
    return ( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
  }
  printf("----mbedtls_ssl_handshake_client_step client_step final:%d\n", ret);
  return ( ret );
}
