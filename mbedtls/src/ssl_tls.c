...
int mbedtls_ssl_fetch_input( mbedtls_ssl_context *ssl, size_t nb_want )
{
    ...
    if ( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    {
        ...
        {
            printf("------mbedtls_ssl_fetch_input time error\n");
        ...
        else
        {
            ...
            printf("------mbedtls_ssl_fetch_input recv:%d\n", ret);
            if ( ret == 0 )
...
#endif
    {
        ...
        while ( ssl->in_left < nb_want )
        {
            ...
            if ( ssl_check_timer( ssl ) != 0 ) {
                ret = MBEDTLS_ERR_SSL_TIMEOUT;
                printf("------mbedtls_ssl_fetch_input MBEDTLS_ERR_SSL_TIMEOUT:%d\n", ret);
            }
            ...
            else
            {
                ...
                else
                {
                    ret = ssl->f_recv( ssl->p_bio,
                                       ssl->in_hdr + ssl->in_left, len );
                    printf("------mbedtls_ssl_fetch_input f_recv:%d\n", ret);
                }
            }
            MBEDTLS_SSL_DEBUG_MSG( 2, ( "in_left: %d, nb_want: %d",
                                        ssl->in_left, nb_want ) );
            ...
            if ( ret < 0 ) {
                printf("------mbedtls_ssl_fetch_input fetch ret < 0:%d\n", ret);
                return ( ret );
            }
...
read_record_header:
    ...
    {
        printf("-----mbedtls_ssl_read_record record fetch err:%d\n", ret);
...
#if defined(MBEDTLS_SSL_CLI_C)
    if ( ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT ) {
        printf("%s\n", "---mbedtls_ssl_handshake_step client");
...
#if defined(MBEDTLS_SSL_SRV_C)
    if ( ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER ) {
        printf("%s\n", "---mbedtls_ssl_handshake_step server");
...
int mbedtls_ssl_handshake( mbedtls_ssl_context *ssl )
{
    ...
    {
        printf("--%s:%d\n", "mbedtls_ssl_handshake begin", ret);
        ret = mbedtls_ssl_handshake_step( ssl );
        printf("--%s:%d\n", "mbedtls_ssl_handshake over", ret);
        if ( ret != 0 )
            break;
    }
